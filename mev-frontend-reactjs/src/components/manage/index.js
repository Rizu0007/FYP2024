...
const Manage = ({ manageData, activeLink }) => {
  return (
    <div className="feature-card rounded-md">
      ...
      <div className="border-2  flex justify-center max-[476px]:block w-full gap-4 border-[#393939] p-5 rounded-r rounded-b">
        {manageData.map((item, index) => (
          <Link
            key={index}
            to={item.link}
            className={`text-[#fff] bg-[#0E1F17] border-[2px] flex justify-between items-center w-fitrounded-lg border-[#589B74] p-3 ${
              activeLink === item.id ? "text-[#589B74]" : " "
            }`}
          >
