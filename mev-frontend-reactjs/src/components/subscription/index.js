...
const Subscription = ({ setShowPopUp }) => {
  const [showBuy, setShowBuy] = useState(false);
  const [timer, setTimer] = useState({})
  const navigate = useNavigate()


  const handleUserFreeTimer = async () => {

    try {
      await API.getUserFreeTimer().then((resp) => {
        if (resp.status === 200) {
          const { days, hours, minutes } = resp.data.data;
  ...
  return (
    <div className="feature-card rounded-md">
      ...
      <div className="border-2 flex flex-col gap-4 border-[#393939] p-5 rounded-r rounded-b">
        ...
        >
          <button onClick={() => setShowPopUp(false)} className="flex justify-start items-center gap-3">
            <svg
