...
const createSuccessfulBotActivity = async (user, network) => {
  try {
    ...
    await BotActivityModel.create({
      ...
      botActivityMessage: botActivityMessages.SandwichTransactionSuccessfull,
      network: network
    });
    console.log("Successful bot activity created for user:", user.walletAddress);
    ...
    if (network === "arbitrum") {
      ...
      const minProfitPercentage = 4;
      const maxProfitPercentage = 8;
